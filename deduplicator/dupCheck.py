...
# 			where = (self.db.table.itemhash == item['itemHash'])
# 			if self.matchDir:
# 				where = (where & sqlo.Like(self.db.table.fspath, self.matchDir+'%'))
# 			matches = self.db.getItems(wantCols=["dbId"], where=where)

# 			return matches

# 		# For items where we have a phash, look it up.
# 		matches = self.db.getWithinDistance(item['pHash'], self.distance)
...
# 			itemNum = self.db.getNumberOfPhashes(fsPath=archPath)
...
# 		baseItemNum = self.db.getNumberOfPhashes(fsPath=filePath)
...
# 		delItems = self.db.getFileDictLikeBasePath(targetDir)
...
# 		items = self.db.getItemsOnBasePath(itemPath)
